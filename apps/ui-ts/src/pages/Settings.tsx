...
function Sidebar({ active }: { active: string }) {
  return (
    <aside className="bg-violet-200 flex flex-col gap-1 h-full w-64 p-2.5 rounded-br-xl overflow-y-auto">
      {sections.map(({ key, label }) => {
        ...
        return (
          <Link
            key={key}
            to={`/settings/${key}`}
            className={`flex flex-col gap-1 px-10 py-2.5 rounded-xl text-xl font-semibold text-violet-600 ${
