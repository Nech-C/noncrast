...
type TimerState = {
  running: boolean;
  timeLeftMs: number;  // in miliseconds
  timeSetMs: number;  // in miliseconds
...
export function TimerProvider({ children }: { children: React.ReactNode }) {
  ...
  const totalTimePassedMs = useRef(0);  // in miliseconds
  ...
  function pause() { 
    ...
    const newTimeLeft = timerState.timeSetMs - totalTimePassedMs.current - sessionTimePassed
    setTimerState((oldState) => {
        return {...oldState, newTimeLeft, running: false}
    })
  }
   ...
   * @param time the number of MILISECONDS 
   */
